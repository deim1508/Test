...
void calculate_external_forces(int *multipler) {
    int i;
...
void calculate_pairwise_forces(int *multipler) {
    int i, j;
    double dx, dy;
    double dr2, dr;
    double f, fx, fy;
...
//    moviefile = fopen("results.mvi", "w");
    ...
    for (t = 0; t < 100000; t++) {
        for (int x = 0; x < rate; x++) {
            thpool_add_work(thpool, (void *) calculate_pairwise_forces, &inters[x]);
        }

        thpool_wait(thpool);

        for (int x = 0; x < rate; x++) {
            thpool_add_work(thpool, (void *) calculate_external_forces, &inters[x]);
        }
